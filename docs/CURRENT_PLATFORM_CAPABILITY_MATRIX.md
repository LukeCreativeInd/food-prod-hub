...
| Production / Live Demand | `/production-demand` | Database/runtime accepted; focused canonical route correction pending redeployment/browser acceptance | Live Migrations 051-052 provide append-oriented mapping-output contributions, fingerprint-idempotent current safe issues, scoped recalculation and stable facility/date/item/UOM aggregate; the shared tenant route list now classifies the page correctly | All four tables remain empty; initial deployment rejected the tenant route and exposed it on central/admin hosts; no live source data, frozen demand, review, delta or plan allocation; Stock On Hand is separate |
