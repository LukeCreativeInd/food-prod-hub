...
| Cross-module / Delivery and production calendars | `/shopify/delivery-zones`, `/shopify/delivery-services`, `/shopify/delivery-calendars`, `/shopify/delivery-parser`, `/shopify/delivery-exceptions` | Live schema and deployed UI; final production acceptance blocked on Auth correction redeploy | Organisation-owned zones, separate customer service/carrier truth, immutable effective calendars, source-order-date parser profiles and versioned production-date/facility interpretation; effective superseded history remains resolvable | Migration 050 and database checks passed with zero rows; Phase 1 parser inputs are order attributes/tags only; no postcode/customer PII, seeded configuration, live parser/order evidence or Production Demand |
