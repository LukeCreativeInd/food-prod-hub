...
Task 234 consumes provider-neutral discovered catalogue evidence through Tenant Admin mapping review. It does not expand Shopify scopes, access credentials, call Shopify, create installations or expose protected customer data. Migration 049 is live/registered, and the mapping UI truthfully remains empty until a controlled connection and catalogue discovery exist.

## Task 235 Delivery Configuration Update

Migration 049 is live/registered. Task 235 adds no Shopify API scope, callback, installation or data import. Connection-specific parser profiles use bounded exact keys and allowlisted formats against privacy-minimised Commerce evidence; no raw payload, exact postcode, address or customer PII is added. Zapiet remains the customer-facing calendar.
