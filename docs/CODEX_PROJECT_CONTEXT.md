...
The active sequence is [EveryBatch Official Roadmap - Tasks 225-348](./225-348-official-roadmap.md). Task 236 remains the current uncommitted task and is database/runtime accepted; Task 237 must not begin before Task 236 deployment/browser acceptance and Luke approval. Review Gate 0 is closed and Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-052 are live/registered. Task 235's delivery/date and Auth foundations are production accepted. Migration 051 adds source-linked contribution history, fingerprint-idempotent safe unresolved evidence and recalculable live Production Demand without review/freeze, plan allocation or seed data. Its runtime pgcrypto resolution defect was corrected by Migration 052 through two `extensions.digest(...)` qualifications while Migration 051 remained immutable. Full rollback-only verification passed with zero residue. No connection, catalogue, mapping, source order, delivery configuration, contribution, demand or frozen-demand row exists, and no downstream Production or Inventory mutation occurred. Stock On Hand remains a separate known issue. Earlier roadmap files are preserved as superseded historical planning.
