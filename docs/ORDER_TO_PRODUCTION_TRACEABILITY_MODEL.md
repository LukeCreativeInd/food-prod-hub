...
The source-to-production chain has production-accepted mapping and delivery-interpretation evidence: external catalogue variant -> approved mapping revision -> immutable mapping outputs or approved exclusion, then source order -> parser/calendar/rule evidence -> reviewed delivery and production dates. Migrations 049-052 are live/registered. Task 236 contribution and live-demand links are database/runtime accepted with deployment/browser acceptance pending; freeze and plan-allocation links remain future stages rather than inferred records.

## Task 236 Implementation Update

Migration 050 is live/registered. Unapplied Migration 051 extends the explicit chain to source line -> current approved mapping output -> selected delivery interpretation -> generation run -> contribution -> live Production Demand key. Contribution rows retain source order/line, mapping/revision/output, delivery interpretation, facility, production date, internal item, exact UOM and calculated quantity. Safe issue rows retain the same identity across unchanged recalculation through a deterministic evidence fingerprint; only a meaningful blocker, exclusion or source-state transition appends replacement history. Freeze and plan-allocation links remain future work.
