...
export async function fetchInventoryReceipts(): Promise<GoodsInwardsListData> {
  ...
  const [suppliersResult, linesResult] = await Promise.all([
    ...
    receiptIds.length > 0
      ? supabase
          ...
          .select("id, receipt_id, status, archived_at")
          .eq("organisation_id", organisationId)
  ...
  ((linesResult.data ?? []) as { receipt_id: string; status: string; archived_at: string | null }[])
    .filter((line) => line.status !== "cancelled" && !line.archived_at)
    .forEach((line) => {
