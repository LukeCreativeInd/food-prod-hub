...
| Cross-module / Delivery and production calendars | `/shopify/delivery-zones`, `/shopify/delivery-services`, `/shopify/delivery-calendars`, `/shopify/delivery-parser`, `/shopify/delivery-exceptions` | Live schema and deployed UI; final production acceptance blocked on final root/Auth correction verification | Organisation-owned zones, separate customer service/carrier truth, immutable effective calendars, source-order-date parser profiles and versioned production-date/facility interpretation; effective superseded history remains resolvable | Migration 050 and database checks passed with zero rows; Auth 429 regression passed three production cycles; isolated root status-400 classification correction awaits deploy/browser verification; no postcode/customer PII, seeded configuration, live parser/order evidence or Production Demand |
