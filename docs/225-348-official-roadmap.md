...
Task 236 remains the current uncommitted task and is database/runtime accepted; it is safe to commit/deploy, but deployment/browser acceptance remains pending and Task 237 has not begun. Task 235 is production accepted through `8d9059c31c11e7019bf610c031b3433cff7ee03b`, `9982a4ee41886702337afc6f3b80947d106155f3` and `f57f2b14ac6774628c3bbb4f45dc7ffc1714dd8c`. Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-052 are live/registered. Migration 051's runtime pgcrypto resolution defect was corrected by Migration 052, and the full rollback-only suite passed with zero residue. Controlled Shopify development-store validation remains a separate later gate; Stock On Hand remains a separate known issue.
...
| 236 | Production Demand Schema and Contribution Generation v1 | **Current/uncommitted; database/runtime accepted.** Adds recalculable source-line contributions, blockers/exclusions, stable live aggregation and read-only UI. Migrations 051-052 are live and the full rollback-only suite passed; deployment/browser acceptance remains pending. | Schema/UI foundation | Yes, `051` and corrective `052` live/registered | Replaces disposable summary totals with traceable live demand; review/freeze/deltas remain Task 237. |
