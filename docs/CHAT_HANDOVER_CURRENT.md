...
- Latest completed repository task: **235 - Delivery Zones, Delivery Calendars and Production-Date Configuration v1**, commit `8d9059c31c11e7019bf610c031b3433cff7ee03b`
...
- Current stage: **Task 235 database acceptance passed; Auth 429 regression passed, but production acceptance is blocked on deploy/browser verification of the final isolated root Auth 400 correction**
- Next approved task after Task 235 production acceptance: **236 - Production Demand Schema Foundation**
...
- Task 235 adds the live tenant-owned delivery configuration foundation and moves Shopify operational configuration under `/shopify`; `/integrations` is now the compact provider catalogue. Historical dates retain effective superseded calendar/parser behavior, parser selection uses source-order business time, and Phase 1 parser sources are exact order attributes/tags only. No zones, services, calendars, parser profiles, source interpretations or overrides are seeded. The Auth request-amplification correction at `9982a4e` passed three complete production navigation cycles. One isolated `GET /` then exposed that Supabase's status-400/code-null `AuthSessionMissingError` was not classified as signed out, while several host modes still reached authenticated `app/page.tsx` before deterministic root routing. The pending final correction adds evidence-based Auth categories, failure-only safe diagnostics and host-mode root routing without changing protected destination guards.
