...
| Production / Live Demand | `/production-demand` | Database/runtime-accepted schema/read UI foundation; deployment/browser acceptance pending | Live Migrations 051-052 provide append-oriented mapping-output contributions, fingerprint-idempotent current safe issues, scoped recalculation and stable facility/date/item/UOM aggregate; full rollback verification passed | All four tables remain empty; no live source data, frozen demand, review, delta or plan allocation; legacy Shopify/Zapiet/CSV flow remains operational; Stock On Hand is a separate known issue |
