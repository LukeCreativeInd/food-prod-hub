...
The active sequence is [EveryBatch Official Roadmap - Tasks 225-348](./225-348-official-roadmap.md). Task 237 is production accepted at `13a5f1b4aca93f0f2fbb38dd256ec5968044ef67` (`Build demand review and freeze workflow`) and deployment `dpl_B7GLzEp5a65YArgHfJRdmciJ2rhy`. Migrations 053-055 are live/registered and immutable. Full rollback-only lifecycle, real independent-session concurrency and production browser verification passed with zero operational residue. Tasks 238-240 are committed, with Task 240 at `a1369117a2d4ebc7ef6ab7b2d819bbaab348e037` (`Plan approved production data collection`). Task 241's corrected repository implementation is under architect review; Migration 056 is unapplied and parser persistence remains dormant, so Task 242 is blocked. Stock On Hand and marketing DNS remain separate.
...
Task 241 now provides the repository-only Production Import source/staging/parser foundation in unapplied Migration 056 and `lib/production-import`. It has no tenant route, canonical mutation, mapping/review/apply flow or real source data. Task 238 is committed at `e23024761f1197997b100a4e26cd401c0f19330a`; Tasks 241-243 must preserve the ownership rules in `238-tools-module-review-production-import-ownership.md` and `TOOLS_AND_PRODUCTION_IMPORT_OWNERSHIP_MATRIX.md`.
...
Task 240 is committed at `a1369117a2d4ebc7ef6ab7b2d819bbaab348e037` and specifies one controlled multi-tab Clean Eats package, stable human transition keys, mandatory field provenance, separate workflow/evidence/confidence states, role-based collection responsibility, blocker-driven readiness and eight transition waves. Formula nominal output, process yield, batch envelope, Method, Work Instruction, QA linkage, Packaging context and water/process classification remain distinct. The workbook is evidence only. Task 241 adds unapplied tenant-owned source/staging/parser foundations and one explicit CSV parser; Formula apply waits for lifecycle hardening and Method/WI apply waits for Task 244. No canonical import, real data, mapping/review/apply UI or Method/WI schema exists.

## Task 241 Production Data Staging And Parser Foundation

Migration 056 defines seven tenant-owned, RLS-enabled evidence tables, a private `production-imports` bucket, exact tenant/run/source object paths, immutable source/parser/staging history and only `production_imports.view`/`production_imports.manage`. Run status is aggregate across all active sources. Upload completion checks available Storage size/optional MIME metadata but remains `uploaded_unverified`; the deterministic parser checks actual UTF-8 checksum, byte size and MIME. Parser start/finalize/fail persistence is dormant and ungranted because no approved trusted runner exists. Storage object policies remain manual. Migration 056 is unapplied pending architect review and Task 242 remains blocked until schema, Storage, runtime and trusted-runner acceptance.
