...
| Tenant Admin / Commerce Integrations | `/integrations` | Real Shopify readiness/list view; production correction pending deployment | Migrations 046-048 are live; Task 233 adds claim and manufacturer-acceptance actions, discovered-catalogue/run summaries and restricted connector runtime | Facility-column/readiness correction is implemented but not browser-accepted; no registered/connected app, live data, mapping or Production Demand; install/sync actions stay gated |
...
| Commerce / Shopify connector | `/shopify`, `/api/integrations/shopify/*` | Non-live implementation foundation; tenant route correction pending deployment | Official auth/GraphQL/HMAC runtime, encrypted credential boundary, durable jobs, reconciliation and live/registered Migrations 047-048; tenant `/shopify` is a read-only permission-gated setup surface | No app registration/listing/review/store validation, scheduler, connection, source order or protected customer data exists |
