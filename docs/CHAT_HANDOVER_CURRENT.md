...
- Current uncommitted repository work: **Task 236 production route-isolation correction**; the main Task 236 commit is deployed, database/runtime accepted and awaiting focused canonical-route redeployment/browser acceptance; suggested commit `Fix Production Demand route isolation`
...
- Task 236 deployed commit hash: `abede6d8596f4da9995c23586f0f70d55cb15efe` (`Build production demand contributions`); focused route-isolation correction remains uncommitted
...
- Current stage: **Task 236 database/runtime accepted; initial deployment found a canonical tenant-route omission; focused correction awaiting commit/redeployment/browser acceptance; all four Task 236 tables remain empty**
- Next approved task after the Task 236 correction is committed, deployed, browser accepted and approved by Luke: **237 - Production Demand Review, Freeze and Delta UI v1**
...
- Migration `051_production_demand_contribution_foundation.sql` is live/registered as `20260806035017 production_demand_contribution_foundation`. Runtime verification found both unqualified pgcrypto `digest` calls failed under fixed `search_path = public`; immutable Migration 051 was corrected by live/registered Migration `052_production_demand_digest_schema_fix.sql` (`20260806081548`), which qualifies only those calls. The full rollback-only suite passed with zero residue. The initial `abede6d` deployment then showed `/production-demand` missing from the canonical tenant route list: tenant access redirected to Dashboard while central/admin exposed the tenant page. The focused list correction changes no data, schema or permission. No Shopify connection, source order, mapping, contribution, live demand, frozen demand or downstream Production/Inventory mutation exists. Task 237 remains blocked pending correction deployment/browser acceptance; Stock On Hand remains separate.
