...
Task 235 is the latest completed repository task at `8d9059c31c11e7019bf610c031b3433cff7ee03b`. Task 234 is committed at `ee755514b2cbbbccd3697d5a14a3f86af148191c`. Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-050 are live/registered. Task 236 is next only after the Task 235 Auth request-rate correction is redeployed, browser accepted and explicitly approved; controlled Shopify development-store validation remains a separate later gate.
...
| 235 | Delivery Zones, Delivery Calendars and Production-Date Configuration v1 | **Complete at `8d9059c31c11e7019bf610c031b3433cff7ee03b`; production acceptance pending Auth correction redeploy/browser verification.** Implements reviewed effective-dated zones, services, calendars, parser profiles, interpretation and date-resolution foundations without seed data or Production Demand. | Schema/configuration/UI foundation | Yes, `050` live/registered | Makes production-date assignment explainable and repeatable; transient Auth failures must not become false business state. |
