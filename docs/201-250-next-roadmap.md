...
| 212 Stock Adjustment/Reversal Plan | Plan controlled stock corrections. | Corrections need ledger-safe workflows after stock visibility exists. | Inventory, QA, Production, Reports | Diagnostics later | Support guide impact | Adjustments/reversals write stock_movements rather than editing summaries | Plan doc | Docs | High |
| 213 Stock Adjustment/Reversal Schema Foundation | Add reviewed correction foundations if needed. | Stores correction metadata safely. | Inventory | None initially | Docs update | Future correction source for Stock On Hand and traceability | SQL migration | Migration | High |
| 214 Stock Adjustment UI v1 | Add controlled adjustment/reversal actions. | Lets authorised users correct stock without direct edits. | Inventory | Diagnostics later | Troubleshooting/release note | Writes ledger rows and should audit correction context | UI/actions/docs | UI | High |
