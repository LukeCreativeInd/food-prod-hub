...
Task 235 is the latest completed repository task; its exact commit hash must be backfilled by Task 236. Task 234 is committed at `ee755514b2cbbbccd3697d5a14a3f86af148191c`. Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-049 are live/registered. Migration 050 is drafted and unapplied. Task 236 is next only after Task 235 review and Migration 050 application/verification; controlled Shopify development-store validation remains a separate later gate.
...
| 235 | Delivery Zones, Delivery Calendars and Production-Date Configuration v1 | **Complete in repository; exact commit hash to be backfilled by Task 236.** Implements reviewed effective-dated zones, services, calendars, parser profiles, interpretation and date-resolution foundations without seed data or Production Demand. | Schema/configuration/UI foundation | Yes, `050` drafted/unapplied | Makes production-date assignment explainable and repeatable once reviewed and applied. |
