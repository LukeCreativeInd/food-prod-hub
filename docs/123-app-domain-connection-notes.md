...
- `admin.everybatchmrp.com` is the preferred future Platform Admin host and remains held until Platform Admin domain routing is implemented.
...
Task 156 documents the separate manual setup checklist for `admin.everybatchmrp.com`. That domain should be connected independently from `app.everybatchmrp.com` and must follow Vercel's exact DNS instructions.
