...
The active sequence is [EveryBatch Official Roadmap - Tasks 225-348](./225-348-official-roadmap.md). Task 235 is the latest completed repository task at `8d9059c31c11e7019bf610c031b3433cff7ee03b`. Review Gate 0 is closed and Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-050 are live/registered. Task 235 adds tenant-safe delivery-zone, service, calendar, parser, interpretation and date-resolution foundations. Historical dates resolve through effective superseded configuration, parser selection uses source-order business time, and Phase 1 parser sources are exact order attributes/tags only. No connection, catalogue, mapping, source order, delivery configuration, Production Demand or seed data exists. Database acceptance passed, but final production acceptance is blocked until the Auth request-rate correction is redeployed and browser verified. Task 236 must not begin before that approval. Earlier roadmap files are preserved as superseded historical planning.
...
The correction caches the server Supabase client and verified user only within the React request tree, uses a browser singleton, suppresses automatic prefetch in dense authenticated shell/Shopify navigation, includes Support in the approved shared production cookie scope and maps only genuine missing/expired/revoked sessions to signed out. Transient Auth failures now produce a safe retry state. RLS, active membership, exact permissions, direct-route guards, host isolation and service-role boundaries remain unchanged. Task 236 is blocked until redeployment and production browser acceptance confirm stable tenant, Support and Platform Admin navigation.
