...
Task 234 is the latest completed repository task; its exact commit hash must be backfilled by Task 235. Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-048 are live/registered. Migration 049 is drafted and unapplied. Task 235 is next only after Task 234 review and Migration 049 application/verification; controlled Shopify development-store validation remains a separate later gate.
...
| 234 | Commerce Product, Variant and Bundle Mapping v1 | **Complete in repository; exact commit hash to be backfilled by Task 235.** Adds direct, bundle/pack and exclusion mapping schema, review lifecycle/history, source-line interpretation readiness and Tenant Admin mapping UI. Migration 049 remains unapplied and no operational data exists. | Schema/UI/configuration | Yes, `049` drafted/unapplied | Replaces hidden exact-title mappings and silent drops once reviewed and applied. |
