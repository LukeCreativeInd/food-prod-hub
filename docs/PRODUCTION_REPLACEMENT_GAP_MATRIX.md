...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Order intake | Already-filtered six-column Zapiet export; no order/line/date/status fields | None; repository verified | Connection, sync events, raw/normalised lines, UI, Shopify integration | Order intake -> Production | Facility routing later; downstream Logistics | A / architecture / provisional sequence | Pre-export source plus change/cancel cases |
...
| Legacy formula/method/instruction transition | Hard-coded Python dictionaries and manual source maintenance; approved current source not supplied | Formula Import planning exists; no legacy Production import | Approved-source inventory, staging/parser, provenance, mapping/review, duplicate/item/UOM/method validation and controlled apply | Tools may support Products/Production-owned records | Import quality affects every Production requirement/task | A/E only if Review Gate 0 activates the conditional unnumbered workstream | Current approved record volume, source structure/quality and manual-entry practicality |
| Yield factors | Fixed quantities, batch overage and water rules exist but are not approved yield data | Formula expected-yield quantity exists | Basic semantics, engine and reviewed assumptions | Products/Production | Changes Inventory requirements and variance | A/E; basic expected-yield calculation is A | Approved current yield/water examples |
...
| Printable fallback | Verified 22-page matched PDF; five section types and repeated copies | No real production export | Versioned area/full export | Production report reader | Facility/area | A/E / parity | Annotated room distribution and outage process |
