...
Task 236 is production accepted through commits `abede6d8596f4da9995c23586f0f70d55cb15efe` and `f344b4ca9a5262b4e7d6967e6ec0c02b0cbe8021`. Task 237 is database/runtime/concurrency accepted and safe to commit/deploy; its exact commit hash must be backfilled by Task 238, while deployment/browser acceptance remains pending. Task 238 is unavailable until that acceptance and Luke approval. Architecture Gate 1 remains approved. Migration 045 is live/browser validated but unregistered; Migrations 046-055 are live/registered. Controlled Shopify development-store validation, Stock On Hand and marketing DNS remain separate.
...
| 237 | Production Demand Review, Freeze and Post-Freeze Delta Workflow v1 | **Database/runtime and concurrency accepted; safe to commit/deploy; deployment/browser acceptance pending.** Captures immutable review evidence, one-owner source commitments, external context, irreversible freeze and owner-filtered exact-UOM cumulative deltas. Exact commit hash to be backfilled by Task 238. | Schema/UI/workflow | Yes, `053` plus corrective `054` and `055` live/registered | Prevents cross-review double counting and unsafe reruns without allocating Production Plans. |
