...
The active sequence is [EveryBatch Rolling Roadmap](./EVERYBATCH_ROLLING_ROADMAP.md). Task 242 is committed at `9fa6ffc017509559976832ff823392ff13574673` (`Establish rolling roadmap governance`) and establishes the concrete 242-250 horizon, separate Reviews, proactive architect intervention and the unnumbered [Candidate Backlog](./EVERYBATCH_CANDIDATE_BACKLOG.md). Task 243 planning is complete and uncommitted; its canonical [Information Architecture](./EVERYBATCH_INFORMATION_ARCHITECTURE.md), [UX Design System](./EVERYBATCH_UX_DESIGN_SYSTEM.md), [Page Pattern System](./EVERYBATCH_PAGE_PATTERN_SYSTEM.md) and [Cross-Module Navigation Model](./EVERYBATCH_CROSS_MODULE_NAVIGATION_MODEL.md) are the implementation handoff for blocked Task 244 and Task 245. Migration 056 remains live/registered as `20260807152024 production_data_staging_parser_foundation`, database/runtime accepted and intentionally dormant for trusted parser persistence. Review 1 is scheduled Wednesday 12 August 2026; time TBD. Stock On Hand and marketing DNS remain separate.

Current onboarding begins with [Current Chat Handover](./CHAT_HANDOVER_CURRENT.md), then the [Rolling Roadmap](./EVERYBATCH_ROLLING_ROADMAP.md), the four Task 243 IA/UX authorities, [Review Register](./REVIEW_REGISTER.md), [Candidate Backlog](./EVERYBATCH_CANDIDATE_BACKLOG.md), task standards, [Support Content Source Register](./SUPPORT_CONTENT_SOURCE_REGISTER.md), [Platform Admin Capability and Diagnostics Register](./PLATFORM_ADMIN_CAPABILITY_AND_DIAGNOSTICS_REGISTER.md), [Master Handbook](./EVERYBATCH_MASTER_HANDBOOK.md), [Engineering Operations](./EVERYBATCH_ENGINEERING_OPERATIONS.md), current matrices, Decision Log and Task Index. The former Tasks 225-348 roadmap, original architect dossier and earlier roadmaps are historical evidence, not current ordering authority.
...
This was the Task 201 sequencing recommendation at the time. It is now historical: Tasks 201-222 are complete and the [EveryBatch Rolling Roadmap](./EVERYBATCH_ROLLING_ROADMAP.md) is the active source for task order. The 201+ prompt template includes source-of-truth and horizon-impact checks. No UI, schema, migration, RLS, permission, auth/domain routing, business logic or package changes were included by Task 201.
...
At the Task 223 checkpoint, the active sequence was Tasks 223-247 with Tasks 248-276 directionally approved, and the then-numbered Task 228 Facility/iPad View was decision-gated. Task 225 later superseded that numbering, and Task 242 later replaced long fixed numbering with the Rolling Roadmap plus Candidate Backlog. New ideas do not alter the active horizon without Luke's approval.
...
Task 223B, Phase 1 Production Replacement and Roadmap Reassessment, is committed at `f8f576603d97732d9fa1f29702fec78fccb05036`. Task 224 is committed at `8b8e94a87f6e94fef78c05317f87cad4bb01caea`. Task 225 is committed at `82a81613556c311198449670b0425106f062a4ef`, closed Review Gate 0 and made `docs/225-348-official-roadmap.md` authoritative at that checkpoint. Task 242 later superseded its fixed future order. Task 226 is committed at `36d53894579e0e8762d7ed441187e5c23552678e`; Task 227 at `fa59c928f8f94a2c320f53144c36d632a140e74c`; Task 228 at `bdd50b0d5890ea58306406d25854adc2d6d32c6c`; Task 229 at `800591a2947fa25f5675f80bc70a6473138ec126`; and Task 230 at `f424817e99990f34447c4822d9d86330b13a38f9`. Migration `045` implements the approved facility foundation and was manually applied through SQL Editor; its live objects/backfill exist, while migration-history reconciliation remains explicit follow-up.
...
The durable success criterion is daily Clean Eats planning and floor execution in EveryBatch without the current production tools/global print-pack dependency, after calculation/instruction parity, real production-day comparison, staff validation, fallback/support and explicit retirement approval. The Rolling Roadmap and Candidate Backlog now govern that programme through capability-based materials and readiness Gates. No migration or runtime behaviour changed in Tasks 223B-225; Task 231 later created and manually applied migration `045`.
...
The audit does not approve legacy formulas, yields, water additions, batch rules, mappings, methods, instructions, room routing or report constants as EveryBatch master data. The current Rolling Roadmap and Candidate Backlog preserve those unresolved decisions and capabilities; roadmap placement never approves a legacy value or implements a capability.

## Task 225 Review Gate 0 Approval And Official Realignment

Luke approved the Tasks 225-348 roadmap at Task 225. That documentation/governance task closed Review Gate 0 and preserved earlier roadmaps. Tasks 226-230 completed the architecture phase, Luke approved Architecture Gate 1 through Task 231, and Demand Gate 2 was satisfied after Task 237. Task 242 later made future gates capability-based: the Materials Gate follows implemented/validated location-aware preparation, and Production Replacement Readiness follows execution, actuals, parity and staff-validation evidence rather than obsolete task numbers.
...
Task 241 now provides the live Production Import source/staging foundation through Migration 056 and deterministic parser code in `lib/production-import`. It has no tenant route, canonical mutation, trusted parser persistence runtime, mapping/review/apply flow or real source data. Task 238 is committed at `e23024761f1197997b100a4e26cd401c0f19330a`; future import candidates and Task 247 must preserve the ownership rules in `238-tools-module-review-production-import-ownership.md` and `TOOLS_AND_PRODUCTION_IMPORT_OWNERSHIP_MATRIX.md`.
...
Task 240 is committed at `a1369117a2d4ebc7ef6ab7b2d819bbaab348e037` and defines the machine collection contract: stable keys, mandatory field provenance, separate workflow/evidence/confidence states, role-based responsibility, blocker-driven readiness and eight transition waves. Formula nominal output, process yield, batch envelope, Method, Work Instruction, QA linkage, Packaging context and water/process classification remain distinct. Task 242 clarifies that one 14-tab workbook is not assumed to be the final human experience; Task 246 will prototype entity-focused collection materials over the same taxonomy. No canonical import, real data, mapping/review/apply UI or Method/WI schema exists.

## Task 241 Production Data Staging And Parser Foundation

Migration 056 defines seven tenant-owned, RLS-enabled evidence tables, a private `production-imports` bucket, exact tenant/run/source object paths, immutable source/parser/staging history and only `production_imports.view`/`production_imports.manage`. Run status is aggregate across all active sources. Upload completion checks backing-object size and observed MIME but remains `uploaded_unverified`; deterministic parser code checks actual UTF-8 checksum, byte size and MIME. Authenticated tenant-safe Storage SELECT/INSERT policies are live. Parser start/finalize/fail persistence remains dormant and ungranted because no approved trusted runner exists. Database/runtime acceptance passed with zero synthetic residue. Migration 057 does not exist.

## Task 242 Rolling Roadmap And Review Governance

Task 242 is committed at `9fa6ffc017509559976832ff823392ff13574673` (`Establish rolling roadmap governance`). It replaces the former fixed future 242-348 sequence with an authoritative rolling horizon of Tasks 242-250 and an unnumbered Candidate Backlog. Every task must report `ROADMAP / HORIZON IMPACT`, inspect relevant unresolved Review findings and let the Product Architect proactively raise architecture, security, sequencing, ownership, UX, permission and operational concerns. Luke still approves material roadmap and architecture changes. Reviews are separately numbered stakeholder checkpoints; Gates remain capability/readiness boundaries. Review 1 is the First Major Staff Review, scheduled Wednesday 12 August 2026 at a time still TBD.

Task 242 also establishes EveryBatch as a governed multi-surface product. Every future numbered task and relevant lettered subtask must explicitly assess Tenant App, Platform Admin, Support / Help Centre and Public / Marketing impact; `No impact` is valid and silence is not. `SUPPORT_CONTENT_SOURCE_REGISTER.md` preserves structured workflow/help/troubleshooting continuity, while `PLATFORM_ADMIN_CAPABILITY_AND_DIAGNOSTICS_REGISTER.md` records minimum safe operator readiness/diagnostic needs. These registers do not grant Platform Admin mutation or Support access to proprietary tenant content.

## Task 243 Information Architecture And UX System

Task 243 planning is complete and uncommitted. It defines `Surface -> Module -> Workspace -> Page type -> Entity/workflow context`, attention-led dashboards, orienting module homes, queue-specific operational presentation, readable Entity Hubs with explicit edit, domain-backed History/provenance and permission-aware cross-module navigation. Shared semantics connect Tenant App, Platform Admin, Support and Public while their shells, density, navigation and data boundaries remain purpose-specific. Task 244 is blocked pending Task 243 review and commit; Task 245 uses the Supplier, Internal Item, Component and Finished Product representatives. No runtime or migration change is included.
