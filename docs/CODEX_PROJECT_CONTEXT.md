...
The active sequence is [EveryBatch Official Roadmap - Tasks 225-348](./225-348-official-roadmap.md). Task 235 is the latest completed repository task; its exact commit hash must be backfilled by Task 236. Review Gate 0 is closed and Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-049 are live/registered. Task 234 is committed at `ee755514b2cbbbccd3697d5a14a3f86af148191c`. Task 235 adds tenant-safe delivery-zone, service, calendar, parser, interpretation and date-resolution foundations through drafted, unapplied Migration 050. Historical dates resolve through effective superseded configuration, parser selection uses source-order business time, and Phase 1 parser sources are exact order attributes/tags only. No connection, catalogue, mapping, source order, delivery configuration, Production Demand or seed data exists. Task 236 follows only after Task 235 review and Migration 050 application/verification. Earlier roadmap files are preserved as superseded historical planning.
