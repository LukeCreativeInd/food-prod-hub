...
| Facilities | Proposed tenant configuration source: `facilities` | Inventory, Production, QA, Logistics, Admin, reports | Organisation-owned physical identity; archive preserves history | `organisation_id` remains tenant boundary; schema is not implemented | Architecture decided; Task 231 gate-blocked |
...
| Inventory receipts and lines | Goods Inwards: `inventory_receipts`, `inventory_receipt_lines` | QA, Inventory, Traceability | Draft editing; posting is controlled and historical | Receipt is a future direct facility root; lines derive and location must match | Operational foundation; Task 231 scope decided |
...
| Production plans and lines | Production: `production_plans`, `production_plan_lines` | Production Admin future, QA future | Status-controlled planning history | Plan is a direct facility root; lines derive; external demand remains upstream evidence | Operational foundation; Task 231 scope decided |
| Production batches and inputs | Production: `production_batches`, `production_batch_inputs` | QA, Inventory output/consumption future, Logistics future | Batch identity and planned inputs preserved | Batch stores facility directly; inputs derive and source locations must match | Schema/UI foundation; Task 231 scope decided |
| Production areas | Production: `production_areas` | Plans/tasks and room views future | Tenant-owned setup | Exactly one facility per area after Task 231 | Schema/readiness foundation |
...
| Commerce providers, storefronts and connections | Proposed Commerce domain from Task 227 | Tenant Admin, Production intake, Platform diagnostics, Support diagnostics | Provider key/store ID is stable; consent, installation, health and archive history are preserved | Storefront, owner, manufacturer and facility remain distinct; no schema exists | Architecture decided; Tasks 229/232-233 implement later |
...
| Source orders and lines | Future provider-agnostic Commerce intake domain | Production planning, CRM, Logistics | Provider owns source lifecycle; EveryBatch preserves source IDs, events and changes | Shopify is first connector, not global schema identity; PII is minimised | Critical future architecture; Tasks 228-233 |
...
| Dispatch runs/deliveries/lines | Logistics: `logistics_dispatch_runs`, `logistics_dispatch_deliveries`, `logistics_dispatch_lines` | Manifests, future delivery issues/reports | Controlled lifecycle and retained history | Dispatch run is direct origin-facility root; children derive | Browser validated foundation; Task 231 scope decided |
