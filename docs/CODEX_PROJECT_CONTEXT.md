...
The active sequence is [EveryBatch Official Roadmap - Tasks 225-348](./225-348-official-roadmap.md). Task 236 is committed/deployed at `abede6d8596f4da9995c23586f0f70d55cb15efe` and database/runtime accepted, but its focused canonical route-isolation correction remains uncommitted; Task 237 must not begin before that correction is deployed, browser accepted and approved by Luke. Review Gate 0 is closed and Architecture Gate 1 is approved. Migration `045_facility_schema_foundation.sql` is live and browser validated but unregistered; Migrations 046-052 are live/registered. Task 235's delivery/date and Auth foundations are production accepted. Migration 051 adds source-linked contribution history, fingerprint-idempotent safe unresolved evidence and recalculable live Production Demand without review/freeze, plan allocation or seed data. Its runtime pgcrypto resolution defect was corrected by Migration 052 through two `extensions.digest(...)` qualifications while Migration 051 remained immutable. Full rollback-only verification passed with zero residue. Initial browser acceptance found `/production-demand` missing from the canonical tenant route list, causing tenant rejection and central/admin exposure; the focused correction registers that route in the shared list without changing data, permissions or page behavior. No connection, catalogue, mapping, source order, delivery configuration, contribution, demand or frozen-demand row exists, and no downstream Production or Inventory mutation occurred. Stock On Hand remains a separate known issue. Earlier roadmap files are preserved as superseded historical planning.
...
Task 236 adds live/registered Migrations 051 and 052 plus a read-only `/production-demand` workspace. The model generates append-oriented source-line contributions only from approved direct or bundle outputs and a resolved delivery interpretation. Explicit exclusions produce no demand; missing, ambiguous, inactive-item, UOM and source-state evidence remains visible as safe issues. Current live demand is a stable projection grouped by organisation, facility, production date, internal item and exact UOM, with full source lineage retained in contribution history. Migration 052 preserves this model and only schema-qualifies the two pgcrypto fingerprint calls. The complete rollback-only suite passed and left all four operational tables empty.

The initial production deployment at `abede6d8596f4da9995c23586f0f70d55cb15efe` exposed a host-classification omission rather than a demand/runtime failure: `/production-demand` was not in `tenantRoutePrefixes`. The focused correction adds it once to the shared canonical list so tenant, central, Platform Admin, Support, marketing and unknown hosts inherit the established isolation policy. No middleware exception, migration, RLS, permission, page, data-helper or operational-data change is included. Production acceptance remains pending correction deployment and browser verification; Task 237 remains blocked.

Tenant recalculation is limited to active members with `production.manage`; reads require active membership and `production.view`. A separate service-role worker entry point exists for future trusted connector orchestration and is not used by tenant UI. The internal source-line generator remains non-executable by `PUBLIC`, `anon`, `authenticated` and `service_role`. No customer PII, title/SKU inference, fake data, Production Plan mutation, batch/task creation, inventory reservation, review, freeze or delta workflow is introduced. Task 236 is database/runtime accepted and safe to commit/deploy but is not production accepted. Task 237 remains the owner of review/freeze/delta behavior after Task 236 deployment/browser acceptance and Luke approval.
