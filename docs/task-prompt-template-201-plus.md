...
For work after Task 239, preserve the canonical production-knowledge model in `PRODUCTION_KNOWLEDGE_CONCEPT_MODEL.md`: Formula/BOM is Products-owned composition; Production Method, Method Step and Work Instruction are Production-owned independently controlled knowledge; Recipe is presentation only; definitions remain separate from Plans, Batches, Tasks and actual records. Do not import process yield into ambiguous Formula yield fields, duplicate nested Component inputs, create a Recipe source of truth or guess source classifications. Task 240 must backfill Task 239's exact commit hash through the post-commit context-delta workflow before beginning implementation planning.
