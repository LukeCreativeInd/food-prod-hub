...
Start with the [current chat handover](docs/CHAT_HANDOVER_CURRENT.md), then the authoritative [Rolling Roadmap](docs/EVERYBATCH_ROLLING_ROADMAP.md), [Information Architecture](docs/EVERYBATCH_INFORMATION_ARCHITECTURE.md), [UX Design System](docs/EVERYBATCH_UX_DESIGN_SYSTEM.md), [Page Pattern System](docs/EVERYBATCH_PAGE_PATTERN_SYSTEM.md), [Cross-Module Navigation Model](docs/EVERYBATCH_CROSS_MODULE_NAVIGATION_MODEL.md), [Review Register](docs/REVIEW_REGISTER.md), [Candidate Backlog](docs/EVERYBATCH_CANDIDATE_BACKLOG.md), [Codex task standards](docs/CODEX_TASK_STANDARDS.md), [Support Content Source Register](docs/SUPPORT_CONTENT_SOURCE_REGISTER.md), [Platform Admin Capability and Diagnostics Register](docs/PLATFORM_ADMIN_CAPABILITY_AND_DIAGNOSTICS_REGISTER.md), [Master Handbook](docs/EVERYBATCH_MASTER_HANDBOOK.md), [Engineering Operations](docs/EVERYBATCH_ENGINEERING_OPERATIONS.md), [Capability Matrix](docs/CURRENT_PLATFORM_CAPABILITY_MATRIX.md), [Source-of-Truth Matrix](docs/MODULE_SOURCE_OF_TRUTH_MATRIX.md), [Decision Log](docs/DECISION_LOG.md) and [Task Index](docs/TASK_INDEX.md). The [Tasks 225-348 roadmap](docs/225-348-official-roadmap.md), [original architect dossier](docs/history/ORIGINAL_ARCHITECT_MEMORY_DOSSIER.md) and earlier roadmaps are historical evidence only.
...
- Task 238 commit: `e23024761f1197997b100a4e26cd401c0f19330a` (`Decide production import ownership`)
- Task 239 commit: `cf2a495786a6efd9cf87372496fcfc71ec766fec` (`Decide production knowledge ownership`)
- Task 240 commit: `a1369117a2d4ebc7ef6ab7b2d819bbaab348e037` (`Plan approved production data collection`)
- Task 242 commit: `9fa6ffc017509559976832ff823392ff13574673` (`Establish rolling roadmap governance`)
- Review Gate 0 is closed; Task 242 makes the [Rolling Roadmap](docs/EVERYBATCH_ROLLING_ROADMAP.md) authoritative for the concrete 242-250 horizon and keeps later capability unnumbered in the [Candidate Backlog](docs/EVERYBATCH_CANDIDATE_BACKLOG.md)
- Task 242 also makes explicit four-surface impact assessment permanent: every future numbered task and relevant lettered subtask records Tenant App, Platform Admin, Support / Help Centre and Public / Marketing impact, including an explicit `No impact` where correct
...
- Task 238 defines Tools as a permanent mixed utility module with strict domain ownership boundaries. Future Production Data Import is a dedicated tenant-owned import/staging domain governed by Production and may be surfaced through Tools; canonical Products and Production records remain owned by their domains.
- Task 240 is complete and committed at `a1369117a2d4ebc7ef6ab7b2d819bbaab348e037`. Task 241 is committed at `8dfc644657c92789dea9831e3f9e51181388cfbb`; Migration 056 is live/registered once as `20260807152024 production_data_staging_parser_foundation` and database/runtime acceptance passed with zero residue. Upload completion remains `uploaded_unverified`, aggregate run status covers every active source, and official parser persistence remains dormant with no PUBLIC/anon/authenticated/service-role execute grant until Task 247 resolves a trusted runner. No Method/WI schema, canonical apply, Production data or tenant UI exists.
- Task 242 is complete and committed at `9fa6ffc017509559976832ff823392ff13574673`; it establishes rolling-horizon governance, a separate Review system, proactive Product Architect intervention, the concrete Tasks 242-250 horizon and an unnumbered Candidate Backlog.
- Task 243 defines the canonical EveryBatch Information Architecture, UX Design System, Page Pattern System and Cross-Module Navigation Model. Its planning is complete and uncommitted; Task 244 remains blocked until Luke reviews and commits it. Review 1 is scheduled for Wednesday 12 August 2026; time TBD.
...
- [Task 239 production knowledge ownership decision](docs/239-formula-method-work-instruction-recipes-ownership-decision.md)
- [Production knowledge concept model](docs/PRODUCTION_KNOWLEDGE_CONCEPT_MODEL.md)
- [Formula, Method, Work Instruction and Recipe ownership matrix](docs/FORMULA_METHOD_WORK_INSTRUCTION_RECIPE_OWNERSHIP_MATRIX.md)
- [Task 240 approved production data collection and import plan](docs/240-approved-production-data-collection-import-plan.md)
- [Task 241 production data staging and parser foundation](docs/241-production-data-staging-parser-foundation.md)
- [Task 242 rolling roadmap governance and Review framework](docs/242-rolling-roadmap-governance-review-framework-task-241-reconciliation.md)
- [EveryBatch rolling roadmap](docs/EVERYBATCH_ROLLING_ROADMAP.md)
- [EveryBatch candidate backlog](docs/EVERYBATCH_CANDIDATE_BACKLOG.md)
- [EveryBatch Review register](docs/REVIEW_REGISTER.md)
- [Production data collection field dictionary](docs/PRODUCTION_DATA_COLLECTION_FIELD_DICTIONARY.md)
- [Clean Eats production data collection package specification](docs/CLEAN_EATS_PRODUCTION_DATA_COLLECTION_PACKAGE_SPEC.md)
- [Production data import readiness matrix](docs/PRODUCTION_DATA_IMPORT_READINESS_MATRIX.md)
- [Clean Eats production data collection responsibility matrix](docs/CLEAN_EATS_PRODUCTION_DATA_COLLECTION_RESPONSIBILITY_MATRIX.md)
- [Clean Eats production data collection waves](docs/CLEAN_EATS_PRODUCTION_DATA_COLLECTION_WAVES.md)
...
- [EveryBatch rolling roadmap (active)](docs/EVERYBATCH_ROLLING_ROADMAP.md)
- [Tasks 225-348 roadmap (historical)](docs/225-348-official-roadmap.md)
