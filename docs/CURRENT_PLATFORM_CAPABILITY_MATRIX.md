...
| Cross-module / Delivery and production calendars | `/shopify/delivery-zones`, `/shopify/delivery-services`, `/shopify/delivery-calendars`, `/shopify/delivery-parser`, `/shopify/delivery-exceptions` | Repository foundation; Migration 050 pending | Organisation-owned zones, separate customer service/carrier truth, immutable effective calendars, source-order-date parser profiles and versioned production-date/facility interpretation; effective superseded history remains resolvable | Phase 1 parser inputs are order attributes/tags only; no postcode/customer PII, seeded configuration, live parser/order evidence or Production Demand; runtime validation follows Migration 050 application |
