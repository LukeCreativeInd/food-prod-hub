...
| Domain | Entity | Current implementation status | Current source of truth | Recommended scope classification | Direct facility identity? | Derivation path | Organisation identity retained? | Historical facility required? | Phase 1 requirement? | Owning future task | Notes/risks |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Commerce | commerce connections (proposed) | Not implemented | Future provider-neutral Commerce | Unresolved pending Tasks 227-230 | Likely optional default target | Connection routing | Yes | Yes for routing evidence | Yes | 227, 229, 232 | Store connection is not facility identity. |
| Commerce | storefronts (proposed) | Not implemented | Future Commerce source identity | External/source-owned with target-facility assignment | No as identity | Connection/routing rule | Yes | Preserve source and resolved target | Yes | 227, 232 | CEA/CEW/Made Active are not facilities. |
...
| Commerce | mappings (proposed) | Partial supplier mappings only; commerce mapping absent | Future Commerce/Products relationship | Organisation-wide with optional connection applicability | No by default | Connection/product mapping | Yes | Mapping version/history required | Yes | 234-235 | Facility override only if product routing differs. |
| Commerce | bundle rules (proposed) | Not implemented | Future Commerce mapping | Organisation-wide with source applicability | No | Storefront/mapping | Yes | Yes | Yes | 234-235 | Exact legacy rules require review. |
